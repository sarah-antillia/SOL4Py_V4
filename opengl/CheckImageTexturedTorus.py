...
class MainView(ZOpenGLMainView):
  ##--------------------------------------------
  class CheckImageTexturedTorus(ZOpenGLTexturedTorus):

    def __init__(self):
      ZOpenGLTexturedTorus.__init__(self, None, None, 0.3, 0.6, 100, 80)
      check_image = ZCheckImage()
      self.bind()

      self.pixelStore(GL_UNPACK_ALIGNMENT, 1)
      self.parameter(GL_TEXTURE_MAG_FILTER, GL_NEAREST) # GL_LINEAR )
      self.parameter(GL_TEXTURE_MIN_FILTER, GL_NEAREST) # GL_LINEAR )
  ...
  class OpenGLView(ZOpenGLView):
    ...
    def initializeGL(self):
      ...
      self.texture = MainView.CheckImageTexturedTorus()
    ...
    def paintGL(self):
      ...
      gluLookAt(2.0, 6.0, 12.0, 0.0, 0.0, 0.0, 0.0, 10.0, 0.0) 
