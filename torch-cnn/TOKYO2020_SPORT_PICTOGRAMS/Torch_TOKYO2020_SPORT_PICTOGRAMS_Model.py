...
#  Torch_TOKYO2020_SPORT_PICTOGRAMS_Model.py
...
from Torch_TOKYO2020_SPORT_PICTOGRAMS_Dataset import *
...
class Torch_TOKYO2020_SPORT_PICTOGRAMS_Model(ZMLModel):
  IMAGE_SIZE = 64 # 128
  
  ##
  # Constructor
  def __init__(self, dataset_id, epochs=0, mainv=None, ipaddress="127.0.0.1", port=7777):
    super(Torch_TOKYO2020_SPORT_PICTOGRAMS_Model, self).__init__(0, mainv)
  ...
  def load_dataset(self, data_root = "./dataset/", 
    ...
    # Load ROADSIGNS
    if self.dataset_id == TOKYO2020_SPORT_PICTOGRAMS:
      # create train/valid datasets
      train_root = data_root + "/train/"
      self.train_dataset = Torch_TOKYO2020_SPORT_PICTOGRAMS_Dataset(root= train_root, 
                            transform=self.train_transformer)
      valid_root = data_root + "/valid/"
      self.valid_dataset = Torch_TOKYO2020_SPORT_PICTOGRAMS_Dataset(root= valid_root, 
...
if main(__name__):
  ...
  try:
    ...
    dataset_id = TOKYO2020_SPORT_PICTOGRAMS
    ...
    if dataset_id == TOKYO2020_SPORT_PICTOGRAMS:
      model = Torch_TOKYO2020_SPORT_PICTOGRAMS_Model(dataset_id, epochs, None)
      model.build()
    else:
